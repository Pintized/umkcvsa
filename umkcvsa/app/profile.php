...
<body>
  ...
  <div class="wrap">
    <div class="card">
      <div class="head">
        <img class="avatar" src="<?= e($pic) ?>" alt="Profile picture">

        <div>
          <h1><?= e(trim(($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? '')) ?: ($user['full_name'] ?? '')) ?></h1>
          <div class="email"><?= e($user['email']) ?></div>
          <span class="role-badges">
            <?php foreach (user_roles($user) as $r): ?>
              <span class="badge <?= e($r) ?>"><?= e(ucfirst($r)) ?></span>
            <?php endforeach; ?>
          </span>
        </div>
      </div>
      ...
      <div class="stats">
        <div class="stat">
          <div class="num"><?= (int)$user['points'] ?></div>
        ...
        <div class="stat">
          <div class="num">0</div>
          <div class="lbl">RSVPs</div>
        </div>
      </div>

      <p class="note">
        Member since <?= e($joined) ?> &middot; This is a preview profile page.
      </p>
    </div>
  </div>
